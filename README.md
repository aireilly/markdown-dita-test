...
A demo repository for authoring DITA content in Markdown (MDITA) instead of XML. MDITA is part of the Lightweight DITA (LwDITA) specification and allows technical writers to use familiar Markdown syntax while producing structured DITA output via the DITA Open Toolkit.

## What's in this repo

* `docs/example.mditamap` -- an MDITA map that references 4 example topics
* `docs/topics/concept-tool-calling.md` -- a **concept** topic explaining vLLM tool calling
* `docs/topics/task-validate-metrics.md` -- a **task** topic with a step-by-step benchmarking procedure
* `docs/topics/reference-server-arguments.md` -- a **reference** topic with a table of key vLLM server arguments
* `docs/topics/kitchen-sink.md` -- a **concept** topic demonstrating MDITA syntax features (inline formatting, lists, tables, code blocks, admonitions, images, links, definition lists, Pandoc header attributes)
* `mkdocs.yml` -- MkDocs Material configuration for previewing the same Markdown source as a static site
* `.vale.ini` and `.vale/` -- Vale linting configuration
...
* Short description as the first paragraph after the H1 heading (not in YAML front matter)
